...
final class MPVKitPlayerCore {
    ...
    private let eventQueue = DispatchQueue(label: "vbox.mpvkit.events", qos: .userInitiated)
    ...
    func load(route: PlaybackRoute) {
        ...
        applyHTTPOptions(headers: route.headers)
        command("loadfile", args: [route.url.absoluteString, "replace"])
        state.errorMessage = nil
        onEvent?(.log("MPVKit 加载：\(route.title)"))
        emitState()
    }
    ...
    func setRate(_ rate: Double) {
        setDouble("speed", rate)
    }

    func setVolume(_ volume: Double) {
        setDouble("volume", max(0, min(volume, 1)) * 100)
    }
    ...
    private func setupMPV() {
        ...
        guard initStatus >= 0 else {
            fail("mpv_initialize 失败：\(errorString(initStatus))")
            mpv_terminate_destroy(handle)
            mpv = nil
            return
        }
        ...
        mpv_set_wakeup_callback(handle, { context in
            ...
            let player = Unmanaged<MPVKitPlayerCore>.fromOpaque(context).takeUnretainedValue()
            player.readEvents()
        }, UnsafeMutableRawPointer(Unmanaged.passUnretained(self).toOpaque()))

        onEvent?(.ready)
        onEvent?(.log("MPVKit 初始化完成"))
    }
    ...
    private func handle(event: mpv_event) {
        ...
        case MPV_EVENT_LOG_MESSAGE:
            ...
            if !text.isEmpty {
                DispatchQueue.main.async { [weak self] in
                    self?.onEvent?(.log("[\(prefix)] \(level): \(text)"))
                }
            }
        case MPV_EVENT_SHUTDOWN:
            DispatchQueue.main.async { [weak self] in
                self?.mpv = nil
                self?.onEvent?(.log("MPVKit 已关闭"))
            }
        default:
            if let name = mpv_event_name(event.event_id) {
                let eventName = String(cString: name)
                DispatchQueue.main.async { [weak self] in
                    self?.onEvent?(.log("MPVKit 事件：\(eventName)"))
                }
            }
        }
    }
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            case MPVKitProperty.timePos:
                if let value = UnsafePointer<Double>(OpaquePointer(property.data))?.pointee {
                    ...
                    self.onEvent?(.progress(current: self.state.currentTime, duration: self.state.duration))
                }
            ...
            case MPVKitProperty.pausedForCache:
                if let buffering = self.readFlag(property.data) {
                    ...
                    self.onEvent?(.buffering(buffering))
                }
    ...
    private func command(_ command: String, args: [String] = [], checkForErrors: Bool = true) {
        guard let handle = mpv else { return }
        var values: [String?] = [command] + args + [nil]
    ...
    private func setDouble(_ name: String, _ value: Double) {
        guard let handle = mpv else { return }
        var data = value
        check(mpv_set_property(handle, name, MPV_FORMAT_DOUBLE, &data), context: name)
    }
    ...
    private func check(_ status: CInt, context: String) {
        if status < 0 {
            let message = "\(context)：\(errorString(status))"
            DispatchQueue.main.async { [weak self] in
                self?.onEvent?(.log("MPVKit API 警告：\(message)"))
            }
        }
    }
    ...
    private func errorString(_ status: CInt) -> String {

...
struct KoreanPornHomeView: View {
    @StateObject private var svc = KoreanPornService.shared
    @State private var categories: [KoreanPornCategory] = []
    ...
    private var videoContent: some View {
        KoreanPornVideoGrid(cid: categories[selectedCateIdx].cid, title: categories[selectedCateIdx].name)
    }
}

// MARK: - 视频网格

struct KoreanPornVideoGrid: View {
    let cid: String
    let title: String
    @StateObject private var svc = KoreanPornService.shared
    @State private var videos: [KoreanPornVideo] = []
    @State private var isLoading = true
                ...
                ScrollView {
                    LazyVGrid(
                        ...
                        ForEach(videos) { video in
                            NavigationLink(destination: KoreanPornPlayerView(video: video)) {
                                KoreanPornVideoCard(cover: video.cover, title: video.title, remarks: video.remarks)
                            }
                            .buttonStyle(.plain)
                            .onAppear {
...
struct KoreanPornVideoCard: View {
    let cover: String
    let title: String
    let remarks: String
...
struct KoreanPornPlayerView: View {
    let video: KoreanPornVideo
    @StateObject private var svc = KoreanPornService.shared
            ...
            await MainActor.run {
                if let url = url {
                    ...
                    vodItem = VodItem(
                        ...
                        vodRemarks: "[福利]韩国色情电影", vodPlayUrl: url
                    )
                } else {

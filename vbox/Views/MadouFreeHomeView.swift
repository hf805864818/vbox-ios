...
struct MadouFreeHomeView: View {
    @StateObject private var svc = MadouFreeService.shared
    @State private var categories: [MadouFreeCategory] = []
    ...
    private var videoContent: some View {
        MadouFreeVideoGrid(cid: categories[selectedCateIdx].cid, title: categories[selectedCateIdx].name)
    }
}

// MARK: - 视频网格

struct MadouFreeVideoGrid: View {
    let cid: String
    let title: String
    @StateObject private var svc = MadouFreeService.shared
    @State private var videos: [MadouFreeVideo] = []
    @State private var isLoading = true
                ...
                ScrollView {
                    LazyVGrid(
                        ...
                        ForEach(videos) { video in
                            NavigationLink(destination: MadouFreePlayerView(video: video)) {
                                MadouFreeVideoCard(cover: video.cover, title: video.title, remarks: video.remarks)
                            }
                            .buttonStyle(.plain)
                            .onAppear {
...
struct MadouFreeVideoCard: View {
    let cover: String
    let title: String
    let remarks: String
...
struct MadouFreePlayerView: View {
    let video: MadouFreeVideo
    @StateObject private var svc = MadouFreeService.shared
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if !video.remarks.isEmpty {
                Label(video.remarks, systemImage: "eye")
    ...
    private func loadPlayURL() {
        ...
        Task {
            let url = await svc.fetchPlayURL(playPath: video.playPath)
            await MainActor.run {
                if let url = url {
                    playURL = url; isLoading = false
                    vodItem = VodItem(
                        vodId: video.vodId, vodName: video.title, vodPic: video.cover,
                        vodRemarks: "[福利]麻豆免费", vodPlayUrl: url
                    )
                } else {

...
struct KanliaoHomeView: View {
    @StateObject private var svc = KanliaoService.shared
    @State private var categories: [KanliaoCategory] = []
    ...
    private var videoContent: some View {
        KanliaoVideoGrid(cid: categories[selectedCateIdx].cid, title: categories[selectedCateIdx].name)
    }
}

// MARK: - 视频网格

struct KanliaoVideoGrid: View {
    let cid: String
    let title: String
    @StateObject private var svc = KanliaoService.shared
    @State private var videos: [KanliaoVideo] = []
    @State private var isLoading = true
                ...
                ScrollView {
                    LazyVGrid(
                        ...
                        ForEach(videos) { video in
                            NavigationLink(destination: KanliaoPlayerView(video: video)) {
                                KanliaoVideoCard(cover: video.cover, title: video.title, remarks: video.remarks)
                            }
                            .buttonStyle(.plain)
                            .onAppear {
...
struct KanliaoPlayerView: View {
    let video: KanliaoVideo
    @StateObject private var svc = KanliaoService.shared
            ...
            await MainActor.run {
                if let url = url {
                    ...
                    vodItem = VodItem(
                        ...
                        vodRemarks: "[福利]今日看料", vodPlayUrl: url
                    )
                } else {

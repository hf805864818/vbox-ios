...
struct HeiliaoHomeView: View {
    @StateObject private var svc = HeiliaoService.shared
    @State private var categories: [HeiliaoCategory] = []
    ...
    private var videoContent: some View {
        HeiliaoVideoGrid(cid: categories[selectedCateIdx].cid, title: categories[selectedCateIdx].name)
    }
}

// MARK: - 视频网格

struct HeiliaoVideoGrid: View {
    let cid: String
    let title: String
    @StateObject private var svc = HeiliaoService.shared
    @State private var videos: [HeiliaoVideo] = []
    @State private var isLoading = true
                ...
                ScrollView {
                    LazyVGrid(
                        ...
                        ForEach(videos) { video in
                            NavigationLink(destination: HeiliaoPlayerView(video: video)) {
                                HeiliaoVideoCard(cover: video.cover, title: video.title, remarks: video.remarks, needsDecrypt: video.needsDecrypt)
                            }
                            .buttonStyle(.plain)
                            .onAppear {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ZStack(alignment: .bottomLeading) {
                HeiliaoCoverImage(url: cover, needsDecrypt: needsDecrypt)
...
struct HeiliaoCoverImage: View {
    ...
    var body: some View {
        Group {
            if let data = decryptedData, let uiImage = UIImage(data: data) {
                Image(uiImage: uiImage)
                    .resizable().aspectRatio(contentMode: .fill)
            } else if decryptFailed && !needsDecrypt {
        ...
        .task {
            if needsDecrypt {
                if let data = await HeiliaoService.shared.fetchDecryptedImageData(for: url) {
                    decryptedData = data
                } else {
                    decryptFailed = true
                }
...
struct HeiliaoPlayerView: View {
    let video: HeiliaoVideo
    @StateObject private var svc = HeiliaoService.shared
    ...
    var body: some View {
        VStack(spacing: 12) {
            HeiliaoCoverImage(url: video.cover, needsDecrypt: video.needsDecrypt)
            ...
            await MainActor.run {
                if let url = url {
                    ...
                    vodItem = VodItem(
                        ...
                        vodRemarks: "[福利]黑料不打烊", vodPlayUrl: url
                    )
                } else {

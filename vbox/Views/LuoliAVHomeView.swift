...
struct LuoliAVHomeView: View {
    @StateObject private var svc = LuoliAVService.shared
    @State private var categories: [LuoliAVCategory] = []
    ...
    private var videoContent: some View {
        LuoliAVVideoGrid(cid: categories[selectedCateIdx].cid, title: categories[selectedCateIdx].name)
    }
}

// MARK: - 视频网格

struct LuoliAVVideoGrid: View {
    let cid: String
    let title: String
    @StateObject private var svc = LuoliAVService.shared
    @State private var videos: [LuoliAVVideo] = []
    @State private var isLoading = true
                ...
                ScrollView {
                    LazyVGrid(
                        ...
                        ForEach(videos) { video in
                            NavigationLink(destination: LuoliAVPlayerView(video: video)) {
                                LuoliAVVideoCard(cover: video.cover, title: video.title)
                            }
                            .buttonStyle(.plain)
                            .onAppear {
...
struct LuoliAVPlayerView: View {
    let video: LuoliAVVideo
    @StateObject private var svc = LuoliAVService.shared
    ...
    private func loadPlayURL() {
        ...
        Task {
            let url = await svc.fetchPlayURL(vodId: video.vodId, pageUrl: video.pageUrl)
            await MainActor.run {
                if let url = url {
                    playURL = url; isLoading = false
                    vodItem = VodItem(
                        vodId: video.vodId, vodName: video.title, vodPic: video.cover,
                        vodRemarks: "[福利]萝莉AV", vodPlayUrl: url
                    )
                } else {
